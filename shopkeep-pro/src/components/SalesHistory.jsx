...
import { Receipt, FileText, Trash2, XCircle } from 'lucide-react';

const SalesHistory = ({ sales, downloadBill, deleteSale, dismissSale }) => {
    return (
                        ...
                        {sales.map(s => (
                            <tr key={s._id} className="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td className="px-6 py-4 text-right">
                                    ...
                                    {s.salesmanName && s.salesmanName !== "Admin" && (
                                        <div className="text-[10px] text-emerald-500 font-bold bg-emerald-50 inline-block px-1.5 py-0.5 rounded mt-1">
                                            Com: ₹{(s.total * 0.05).toFixed(2)} | By {s.salesmanName}
                                        </div>
                                    )}
                                </td>
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-2">
                                        ...
                                        {/* Remove Button for Salesman Dashboard */}
                                        {dismissSale && (
                                            <button
                                                onClick={() => {
                                                    if (window.confirm("Remove this sale from your view?")) {
                                                        dismissSale(s._id);
                                                    }
                                                }}
                                                className="bg-orange-50 text-orange-500 p-2 rounded-xl hover:bg-orange-500 hover:text-white transition-all shadow-sm flex items-center gap-2 text-xs font-bold"
                                                title="Hide from Your Dashboard"
                                            >
                                                <XCircle size={16} />
                                            </button>
                                        )}
